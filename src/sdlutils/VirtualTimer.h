...
public:
	...
	// periods.
	inline Uint64 currRealTime() const {
		return _paused ?
	...
	//
	inline Uint64 regCurrTime() {
		if (!_paused) {
			Uint64 currTime = currRealTime();
